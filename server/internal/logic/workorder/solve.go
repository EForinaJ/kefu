...
// Solve implements service.IWorkOrder.
func (s *sWorkOrder) Solve(ctx context.Context, id int64) (err error) {
	...
	_, err = dao.SysWorkOrder.Ctx(ctx).
		Where(dao.SysWorkOrder.Columns().AssigneeId, ctx.Value("userId")).
		Data(g.Map{
			dao.SysWorkOrder.Columns().Status: consts.WorkOrderStatusResolved,
		}).Update()
	if err != nil {
